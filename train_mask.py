...
parser.add_argument("--teacher_score", type=str, default=None,
                    help="Path to teacher_score.pt from teacher_scoring.py. "
                         "Provides a stable per-vocab difficulty prior that anneals "
                         "into the student's own accuracy signal over training.")
parser.add_argument("--teacher_only", action="store_true",
                    help="If set, masking weights are driven entirely by teacher_score "
                         "for the whole run (alpha=0 always) — no transition to student signal.")
...
def update_mask_weights(mask_weights, mask_stats, mlm_prob=0.15,
                        teacher_score=None, alpha=1.0):
    """
    alpha=0 → full teacher prior (start of training, student signal noisy)
    alpha=1 → full student signal (end of training, identical to original)
    """
    student_score = (mask_stats['correct'] + 0.5) / (mask_stats['incorrect'] + mask_stats['correct'] + 1)
    # if there are no instances, treated as 50% accuracy
    # things that never show up will go to 7.5% mlm_prob (seems fine?)

    if teacher_score is not None and alpha < 1.0:
        # normalise teacher_score to [0, 1] for mixing with student_score
        ts_min = teacher_score.min()
        ts_max = teacher_score.max()
        ts_norm = (teacher_score - ts_min) / (ts_max - ts_min + 1e-8)
        ts_norm = ts_norm.to(student_score.device)
        # student_score is EASE (high = token is easy, from accuracy).
        # teacher_score is DIFFICULTY (high = token is hard to predict).
        # Blending them as-is treats "hard" as "easy" and masks difficult
        # tokens LESS often -- the opposite of the intended curriculum.
        # Invert to a shared "ease" polarity before blending.
        ts_ease = 1.0 - ts_norm
        combined_score = alpha * student_score + (1.0 - alpha) * ts_ease
    else:
        combined_score = student_score

    new_weights = mlm_prob - (combined_score * mlm_prob)
...
def train(args, model, tokenizer, train_dataloader, eval_dataloader, teacher_score=None):
    if args.flops:
        from fvcore.nn import FlopCountAnalysis


    if args.lamb:
        optimizer = LAMB(model.parameters(), lr=args.lr, betas=(0.9, 0.95), eps=1e-08, weight_decay=args.weight_decay)
    else:
    ...
    args.mlm_prob_init = args.mlm_prob  # keep original target for phased teacher/decay schedule
    mask_stats = {
        ...
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (
                    ...
                    if args.soft:
                        ...
                        if args.teacher_only:
                            # pure teacher signal for the entire run, no student contribution
                            alpha = 0.0
                        else:
                            # Phased schedule: first 50% of training does teacher->student
                            # alpha annealing with mlm_prob held fixed; second 50% holds
                            # alpha=1 (student-only) and applies mask decay. This avoids
                            # the two mechanisms shifting mask_weights simultaneously,
                            # which caused loss explosions when run in parallel.
                            half_steps = args.total_steps * 0.5
                            if global_step < half_steps:
                                alpha = global_step / half_steps
                            else:
                                alpha = 1.0

                        mask_weights = update_mask_weights(
                ...
                if is_step("logging", global_step, args):
                    ...
                    print(f"Epoch {epoch_float:.2f}, Loss: {loss.item():.4f}, LR: {scheduler.get_last_lr()[0]:.2e}, mlm_prob: {args.mlm_prob:.4f}", flush=True)
                ...
                if args.mask_decay > 0:
                    if args.teacher_only:
                        # no phase split needed — teacher drives mask shape the whole run,
                        # decay just lowers the global average like in the paper
                        args.mlm_prob = args.mlm_prob - (args.mask_decay / args.total_steps)
                    else:
                        half_steps = args.total_steps * 0.5
                        if global_step >= half_steps:
                            # decay only over the second half, so total decay by
                            # end of training still equals args.mask_decay
                            args.mlm_prob = args.mlm_prob - (args.mask_decay / half_steps)
...
def main():
    ...
    # Load teacher prior if provided
    teacher_score = None
    if args.teacher_score and os.path.exists(args.teacher_score):
        teacher_score = torch.load(args.teacher_score).to(dtype=torch.float32)
        print(f"Loaded teacher_score from {args.teacher_score} "
              f"(vocab_size={teacher_score.shape[0]})")
    elif args.teacher_score:
        print(f"Warning: --teacher_score path not found: {args.teacher_score}, ignoring.")
